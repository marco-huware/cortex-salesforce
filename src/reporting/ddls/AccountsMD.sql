...
CREATE OR REPLACE VIEW `{{ project_id_tgt }}.{{ dataset_reporting_tgt_sfdc }}.AccountsMD`
  OPTIONS( description = 'View for Accounts Table')
AS (
  SELECT
    Accounts.AccountId,
    ...
    Accounts.Type,
    Accounts.Website
  FROM `{{ project_id_src }}.{{ dataset_cdc_processed_sfdc }}.accounts` AS Accounts
);
