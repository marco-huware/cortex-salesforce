...
CREATE OR REPLACE VIEW `{{ project_id_tgt }}.{{ dataset_reporting_tgt_sfdc }}.ContactsMD`
  OPTIONS(description = 'View for Contacts Table')
AS (
  SELECT
    Contacts.ContactId,
    Contacts.AccountId,
  ...
  FROM
    `{{ project_id_src }}.{{ dataset_cdc_processed_sfdc }}.contacts` AS Contacts
);
